...
\begin{definition}\label{MonomialOrder.leadingTerm}
  \lean{Groebner.Defs.MonomialOrder.leadingTerm}
  \leanok
  Given a nonzero polynomial \( f \in k[x] \), let
\[
f = c_0 x^m + c_1 x^{m-1} + \cdots + c_m,
...
\begin{definition}\label{MonomialOrder.IsRemainder}
  \lean{Groebner.Defs.MonomialOrder.IsRemainder}
  \leanok
  Fix a monomial order \(>\) on \(\mathbb{Z}_{\geq 0}^n\), and let
\(F = (f_1, \ldots, f_s)\) be an ordered \(s\)-tuple of polynomials in \(k[x_1, \ldots, x_n]\).
Then every \(f \in k[x_1, \ldots, x_n]\) can be written as
...
\begin{definition}\label{MonomialOrder.sPolynomial}
  \lean{Groebner.Defs.MonomialOrder.sPolynomial}
  \leanok
  The $S$-polynomial of $f$ and $g$ is the combination
\[
S(f, g) = \frac{x^\gamma}{\mathrm{LT}(f)} \cdot f - \frac{x^\gamma}{\mathrm{LT}(g)} \cdot g.
...
\begin{definition}\label{MonomialOrder.IsGroebnerBasis}
  \lean{Groebner.Defs.MonomialOrder.IsGroebnerBasis}
  \leanok
  Fix a monomial order on the polynomial ring $k[x_1, \ldots, x_n]$.A finite subset $G = \{g_1, \ldots, g_t\}$ of an ideal $I \subseteq k[x_1, \ldots, x_n]$, with $I \ne \{0\}$, is said to be a \textbf{Gröbner basis} (or standard basis) if
\[
\langle \operatorname{LT}(g_1), \ldots, \operatorname{LT}(g_t) \rangle = \langle \operatorname{LT}(I) \rangle.
...
\begin{lemma}\label{MonomialOrder.zero_le}
  \lean{Groebner.Defs.MonomialOrder.zero_le}
  \leanok

\end{lemma}

\begin{lemma}\label{MonomialOrder.IsRemainder_def'}
  \lean{Groebner.Defs.MonomialOrder.IsRemainder_def'}
  \leanok
...
\begin{lemma}\label{MonomialOrder.lm_eq_zero_iff}
  \lean{Groebner.Defs.MonomialOrder.lm_eq_zero_iff}
  \leanok
...
\begin{lemma}\label{MonomialOrder.leadingTerm_image_sdiff_singleton_zero}
  \lean{Groebner.Defs.MonomialOrder.leadingTerm_image_sdiff_singleton_zero}
  \leanok
...
\begin{lemma}\label{MonomialOrder.isRemainder_of_insert_zero_iff_isRemainder}
  \lean{Groebner.Defs.MonomialOrder.isRemainder_of_insert_zero_iff_isRemainder}
  \leanok
...
\begin{lemma}\label{MonomialOrder.isRemainder_of_singleton_zero_iff_isRemainder}
  \lean{Groebner.Defs.MonomialOrder.isRemainder_of_singleton_zero_iff_isRemainder}
  \leanok
  Let $p \in R[\mathbf{X}]$ be a polynomial, $G'' \subseteq R[\mathbf{X}]$ a set of polynomials,
and $r \in R[\mathbf{X}]$ a remainder. Then the remainder property is invariant under
...
\begin{lemma}\label{MonomialOrder.sPolynomial_antisymm}
  \lean{Groebner.Defs.MonomialOrder.sPolynomial_antisymm}
  \leanok
...
\begin{lemma}\label{MonomialOrder.sPolynomial_eq_zero_of_left_eq_zero}
  \lean{Groebner.Defs.MonomialOrder.sPolynomial_eq_zero_of_left_eq_zero}
  \leanok
...
\begin{lemma}\label{MonomialOrder.sPolynomial_eq_zero_of_right_eq_zero'}
  \lean{Groebner.Defs.MonomialOrder.sPolynomial_eq_zero_of_right_eq_zero'}
  \leanok
  For any polynomial $g \in \MvPolynomial{\sigma}{R}$ and monomial order $m$,
...
\begin{lemma}\label{MonomialOrder.div_set'}
  \lean{Groebner.Defs.MonomialOrder.div_set'}
  \leanok
...
\begin{lemma}\label{Ideal.fg_span_iff_fg_span_finset_subset}
  \lean{Groebner.Ideal.Ideal.fg_span_iff_fg_span_finset_subset}
...
\begin{lemma}\label{Ideal.span_singleton_zero}
  \lean{Groebner.Ideal.Ideal.span_singleton_zero}
  \leanok
...
\begin{lemma}\label{Ideal.span_insert_zero}
  \lean{Groebner.Ideal.Ideal.span_insert_zero}
  \leanok
...
\begin{lemma}\label{Ideal.span_sdiff_singleton_zero}
  \lean{Groebner.Ideal.Ideal.span_sdiff_singleton_zero}
  \leanok
...
\begin{lemma}\label{MonomialOrder.leadingTerm_ideal_span_monomial}
  \lean{Groebner.Ideal.MonomialOrder.leadingTerm_ideal_span_monomial}
  \leanok
...
\begin{lemma}\label{MonomialOrder.mem_ideal_of_remainder_mem_ideal}
  \lean{Groebner.Ideal.MonomialOrder.mem_ideal_of_remainder_mem_ideal}
  \leanok
...
\begin{lemma}\label{MonomialOrder.remainder_mem_ideal_iff}
  \lean{Groebner.Ideal.MonomialOrder.remainder_mem_ideal_iff}
  \leanok
...
\begin{lemma}\label{MonomialOrder.remainder_sub_remainder_mem_ideal}
  \lean{Groebner.Ideal.MonomialOrder.remainder_sub_remainder_mem_ideal}
  \leanok
  Let \( I \subseteq k[x_i : i \in \sigma] \) be an ideal, and let \( G \subseteq I \) be a finite subset.
...
\begin{lemma}\label{MonomialOrder.exists_groebner_basis}
  \lean{Groebner.Basic.MonomialOrder.exists_groebner_basis}
  \leanok
  Let \( I \subseteq k[x_1, \ldots, x_n] \) be an ideal. Then there exists a finite subset \( G = \{g_1, \ldots, g_t\} \) of \( I \) such that \( G \) is a Gröbner basis for \( I \).
-
\end{lemma}

\begin{lemma}\label{MonomialOrder.groebner_basis_isRemainder_zero_iff_mem_span}
  \lean{Groebner.Basic.MonomialOrder.groebner_basis_isRemainder_zero_iff_mem_span}
...
\begin{lemma}\label{MonomialOrder.is_groebner_basis_iff}
  \lean{Groebner.Basic.MonomialOrder.is_groebner_basis_iff}
...
\begin{lemma}\label{MonomialOrder.groebner_basis_is_basis}
  \lean{Groebner.Basic.MonomialOrder.groebner_basis_is_basis}
...
\begin{lemma}\label{MonomialOrder.buchberger_criterion}
  \lean{Groebner.Basic.MonomialOrder.buchberger_criterion}
...
\begin{lemma}\label{Set.finset_subset_preimage_of_finite_image}
  \lean{Groebner.Set.Set.finset_subset_preimage_of_finite_image}
  \leanok
...
\begin{lemma}\label{Submodule.span_sdiff_singleton_zero}
  \lean{Groebner.Submodule.Submodule.span_sdiff_singleton_zero}
  \leanok
  \[
